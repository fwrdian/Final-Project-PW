...
const KatalogDetail = ({ isLoggedIn = false }) => { 
  ...
  const handleOrder = (car) => {
    ...
    if (!isLoggedIn) {
      ...
      // Oper data mobil ke halaman login agar setelah login bisa langsung diarahkan
      navigate('/login', { 
        state: { intendedCar: car } 
      });
    } else {
      // Jika sudah login, langsung lempar ke halaman contact dengan auto-message
      navigate('/contact', { 
            ...
            <div key={unit.id} className="group bg-white border border-slate-100 rounded-3xl overflow-hidden hover:shadow-2xl transition-all duration-500 flex flex-col">
              ...
              <div className="p-8 flex flex-col flex-grow">
                ...
                <div className="mt-auto pt-6 border-t border-slate-50 flex justify-between items-center">
                   <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{unit.engine ? unit.engine.split(' ')[0] : 'Premium'} Engine</span>
                   <button 
